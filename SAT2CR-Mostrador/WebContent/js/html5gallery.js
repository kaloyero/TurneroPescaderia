     ...
     //           if (a.options.fv || 0 < a.options.watermarklink.length) a.options.watermarkcode += "</a>";
            ...
            initData: function(a) {
                ...
                if (this.options.src && 0 < this.options.src.length) {
                	...
                	console.log("VAL ESesss",b)

                    for (var d =
                            0; d < b["lists"].length; d++) {
                    	console.log("VAL ES",b,d)
                        var e = b["lists"][d].mediatype ? b["lists"][d].mediatype : c.checkType(b["lists"][d].file);
                        c.elemArray.push([d, b["lists"][d].thumbnail, b["lists"][d].file, b["lists"][d].fileogg, b["lists"][d].filewebm, b["lists"][d].link, b["lists"][d].linktarget, b["lists"][d].title, b["lists"][d].description, e, b["lists"][d].width && !isNaN(parseInt(b["lists"][d].width)) ? parseInt(b["lists"][d].width) : c.options.width, b["lists"][d].height && !isNaN(parseInt(b["lists"][d].height)) ? parseInt(b["lists"][d].height) : c.options.height, b["lists"][d].poster, b["lists"][d].hd, b["lists"][d].hdogg, b["lists"][d].hdwebm, b["lists"][d].duration, b["lists"][d].lightboxwidth, b["lists"][d].lightboxheight, b["lists"][d].youtubeapikey, b["lists"][d].youtubeplaylistid,
                            b["lists"][d].youtubeplaylistmaxresults, b["lists"][d].lightbox
                        ])
                    }
                    c.readTags();
                    a(c)
